...
<div class="fixed bottom-0 left-0 z-10 flex w-full items-center justify-center">
	<div class="relative w-full sm:w-lg lg:w-2xl">
		<div class="absolute bottom-0 left-full mb-16 -ml-15 flex flex-col gap-2 sm:mb-4 sm:ml-4">
			<a href={resolve('/')} title="home" class="size-11 rounded-full border border-black bg-white">
			...
			<button
				...
				class="rounded-full border border-black bg-white p-2 transition-opacity hover:opacity-70"
			...
			<button
				title="toggle grayscale"
				class={twMerge(
					'rounded-full border border-black bg-white p-2 font-bold transition-opacity hover:opacity-70',
					!nsfw && 'line-through'
				)}
				on:click={toggleNSFW}
			>
